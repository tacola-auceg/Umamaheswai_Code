		...
		//	llq=TamilQueryGraph.multilist_UNLQuery();//get translated query
			...
			for(int i=0;i<1;i++) //Interating the search process for each query and it's expansion
			{
				...
				while(cpt1 != null)
				{
						...
						//System.out.println("Input for c results"+tamil_c1+c1+c1pos+c1_qtag);
						...
						//System.out.println("The toUW concept"+ToCpt1.uwtoconcept.trim());
						...
						//System.out.println("c2"+c2+"<--------conid---->"+conid+"\t"+tamilid);
						if(!c1.equals(c2)){
						...
						////////System.out.println("c2tag"+to_con_tag+"conid"+tamil_c2+"\t"+c2pos);
						c2concpt.add(c2);
						...
						//System.out.println("Inside c2 only"+c2);
LUS._searchC1C2("/root/Desktop/Lucene-UWDict-Index/",c2,"c2",tamil_c2,c2pos,to_con_tag,mw_tag,Integer.parseInt(conid),tamilid,flag_singleword,queryWord);
	...
	//System.out.println("Inside c1 only"+c1);
...
if(!conList.containsKey(c1)&&!conList.containsKey(c2)){
						...
						//System.out.println("Inside CRC"+Integer.parseInt(cpt1.con_uid));
LUS._searchCRC("",tamil_c1,c1,relation,c2,tamil_c2,mw_tag,c1_qtag,to_con_tag,Integer.parseInt(cpt1.con_uid),queryWord);
				 ...
				 if((!(cpt1.poscheck.equals("Verb"))) && (!(cpt1.poscheck.equals("Finite Verb"))&& (!(cpt1.poscheck.equals("Adjective"))) && (!(cpt1.poscheck.equals("Particle"))) && (!(cpt1.poscheck.equals("Postposition")))&& (!(cpt1.poscheck.equals("Adjectival Noun"))) ))
					{			
						...
						else
						{
							...
							if(set1.add(cpt1.uwconcept)){
							//System.out.println("No relation"+cpt1.uwconcept);
LUS._searchC1C2("/root/Desktop/Lucene-UWDict-Index/",cpt1.uwconcept,"c1",cpt1.gn_word.trim(),cpt1.poscheck,cpt1.queryTag,cpt1.MWtag_Qw.trim(),Integer.parseInt(cpt1.con_uid),cpt1.tam_uid,flag_singleword,queryWord);
							}
							}
		...
		/*		////////System.out.println("con_Only"+con_Only);
		if(con_Only){
		...
		for(int i=0;i<1;i++) //Interating the search process for each query and it's expansion
			{
				...
				{
						limiter++;
					////////System.out.println("Now llq is not null\t"+cpt1.uwconcept+cpt1.poscheck);
	...
	public void querywordHashtable(FinalLLImpl[] llq)
	{
				...
				while(cpt1 != null)
				{
					...
					//	//////////////////////System.out.println("hashtable"+cpt1.uwconcept+cpt1.gn_word);
		...
		public static ArrayList seperateTag(ArrayList list)
	{
		...
		//if(tag1.size()!=0)
		{
			////////////System.out.println("Inside First Tag");
			finalList.addAll(tag1);
		}
		//if(tag23.size()!=0)
		{
			////////////System.out.println("Inside Second Tag");
			finalList.addAll(tag23);
		}
		//if(tag4.size()!=0)
		{
			////////////System.out.println("Inside Fourth Tag");
			finalList.addAll(tag4);
		}
	//if(tag56.size()!=0)
		{
			////////////System.out.println("Inside Fifthandsixth Tag");
			finalList.addAll(tag5);
		}
		//if(tag7.size()!=0)
		{
			////////////System.out.println("Inside Seventh Tag");
